...
export class JuegoComponent implements OnInit, OnDestroy {
    ...
    if (this.sesion.enjuego == 0) {
      this.servicioMazo.getMazo().subscribe({
        next: (data) => {
          ...
          this.iniciarJuego();
          this.servicioSesion.juegoNuevo(this.sesion).subscribe();
        }
      })
    } else {
      this.obtenerJuego();      
  ...
  calcularVictoria() {
    this.sesion.totalDePartidas++;    
    // 1 - Se paso | 2 - Empate | 3 - Derrota | 4 - Victoria  
    //blackjack jugador y victoria
    if (this.puntajeJugador == 21 && this.puntajeCrupier != 21) {
      this.resultado = 5;
      this.sesion.victoriasJugador++;
      this.sesion.blackjackJugador++;
      return;
    }
    //se paso de 21
    if (this.puntajeJugador > 21) {
      this.resultado = 1;
      this.sesion.sePasoDe21++;
      return;
    }
    // empate (posible 21)
  ...
  actualizarJuego() {    
    this.servicioSesion.ActualizarStats(this.sesion).subscribe();    
